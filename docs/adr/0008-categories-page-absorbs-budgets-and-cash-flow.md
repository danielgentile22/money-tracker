---
status: accepted — amends [ADR-0002](0002-local-web-app-typescript-fullstack.md) (surface layout)
date: 2026-07-06
---
...
buried in Settings. How should these surfaces be consolidated? (Grill session 2026-07-06,
second session.)

## Considered Options

* One month-first Categories page absorbing Budgets, Cash Flow, and category management
* Merge Recurring in too (owner's first instinct)
* A literal "Recurring" Category re-homing rent/Netflix/etc.
* Filter-set-first page with the budget column appearing only on exact-month ranges

## Decision Outcome

Chosen option: one **Categories** page (`/categories`) carries the month's Sankey, a ranked
group→category list with hard budget lines, per-category drill-down into a full Ledger, and
all category/group management. `/budgets` and `/cash-flow` redirect there.
...
* **Month-first, not Filter-set-first.** Every other analysis view speaks the Filter set;
  ...
  `(Category, month)` with fill-forward — an arbitrary date range renders the budget column
  meaningless, and the budget column is the centerpiece. Arbitrary ranges remain the
  Reports page's job.
...
Rejected: Recurring series are keyed on Merchant, not Category, and their states derive
from *today*, which a month cursor contradicts. Recurring stays a standalone page;
recurring-ness is an attribute a Transaction has in addition to its Category.

### A literal "Recurring" Category re-homing rent/Netflix/etc.

Rejected: it would erase the category identity of the largest charges and defeat the
